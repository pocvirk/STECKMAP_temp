...
func mlabel(s,p1,p2,c=,t=,w=,h=,np=,dx=,dy=,auto=,yshift=,xshift=,slog=){
  /* DOCUMENT
  ...
  */
  ...
  if(slog==1) write,"using log y placement";
  ya=(slog==0)?span(p2(2),p1(2),nl)(,-:1:np):spanl(p2(2),p1(2),nl)(,-:1:np);
...
//      c(,i); // debug only
    ...
    add_txt,s(i),pos=[xa(0)+dx,((slog==1)?ya(i,1)+dy:ya(i,1)*(1.+dy))],sys=1,height=h(i);
...
func PLmlabel(s,p1,p2,c=,t=,w=,np=,dx=,dy=,inc=,m=,ms=,h=,slog=,auto=,yshift=,stretch=,tosys=){
  /* DOCUMENT
  ...
  */
  ...
  for(i=1;i<=nl;i++){
    ...
    //write,xa;
    //write,i;
    add_txt,s(i),pos=[xa(0)+dx,((slog==0)?ya(i,1)+dy:ya(i,1)*(1.+dy))],sys=tosys,height=h(i);
...
func xyleg(s1,s2,s3,cs1=,cs2=,cs3=,dx1=,dy1=,dx2=,dy2=,dx3=,dy3=,height=){
  /* DOCUMENT
  ...
  */
  ...
  plt,s1,cs1(1)+dx1,cs1(2)+dy1,tosys=0,height=height;
  if (!is_void(s2)) plt,s2,cs2(1)+dx2,cs2(2)+dy2,tosys=0,orient=1,height=height;
  if(!is_void(s3)) plt,s3,cs3(1)+dx3,cs3(2)+dy3,tosys=0,orient=1,height=height;
};
...
func xnpal(xn)
/*DOCUMENT
  returns rgb for given xn (neutral fraction), trying to make a table that starts with gray for x~1 and goes to red for x~0.5 and then white for x~0.
  physically this is to suggest opaque medium for x~1, then medium radiating through recombination and finally totally transparent, no recombination medium
  Actually from a physical point of view it woudl make more sense to make it red for x=1 and blue for x=0 since the neutral medium absorbs in the blue...
*/
{
  xt1=0.5;
  //  xt1=0.3333;
  ...
  if(xn>=x1) c=c1+((c2-c1)/(x2-x1))*(xn-x1);

    //c=c0+[1.,0.,0.]*(1.-c0(1))*(xn-xt0)/(1.-xt0)+[0.,1.,1.]*(0.-c0(2))*(xn-xt0)/(1.-xt0);
  if(xn<x1) c=c0+((c1-c0)/(x1-x0))*(xn-x0);
  return c;
}

func writepalette(file){
  ncolors=240;
  x=span(0.,1.,ncolors);
